...
namespace PasswordEncryptor {
    public class HashSalt {
        ...
        public static string StringtoHashSalt(string plainText) {
        ...
        public static bool CompareInputtoPassword(string plainTextInput, string plainTextHashed) {
            // Create a variable for comparing
            bool isEqual = false;

            // Converting the password to a hash
            byte[] hashBytes = Convert.FromBase64String(plainTextHashed);
            ...
            var pbkdf2 = new Rfc2898DeriveBytes(plainTextInput, salt, 10000);
            byte[] hash = pbkdf2.GetBytes(20);
